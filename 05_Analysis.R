...
for(matched_set in names(instruction_sets)){
  ## create permutation group ----
  set.seed(123) # may not be needed as not random?
  matched_sets[[matched_set]] <- matched_sets[[matched_set]] %>%
  ...
  # now join with young
  exp_temp <- left_join(matched_sets[[matched_set]], exp_temp)
  ...
  analytic <- left_join(analytic, 
                        d$old %>% select(CASE_ID_OLD_RA, all_of(
                          paste0(instruction_sets[[matched_set]]$exact,"_HRS_RA"))),
                        relationship = "many-to-many")
  ...
  for (i in 1:10){
    ...
    ## Run models ----
    models[[matched_set]][[paste0("p_",i)]] <- lm(GENHEALTH_HRS_14 ~ BMI_HRS_2 + age_dec50 + 
  ...
  # extract coefs
  coefficients<- as.data.frame(lapply(models[[matched_set]], coef))
  # find mean coef
  rubin[[matched_set]]$coef_mean <- apply(coefficients, 1, mean) 
  ...
  variance <-as.data.frame(
    lapply(models[[matched_set]], 
           function(x){(summary(x)$coefficients[,2])^2}))
  average_var <- apply(variance, 1, mean)
  
  # sum variances
  rubin[[matched_set]]$final_variance <- coef_variance + average_var
}
